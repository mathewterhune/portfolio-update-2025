...
export default function Projects() {
  ...
  return (
    <section id="projects" className="w-full p-4 sm:p-6 lg:p-10">
      <motion.h1
        className="text-2xl sm:text-3xl lg:text-4xl text-gray-700 font-bold mb-6 sm:mb-8"
      ...
      <div className="grid grid-cols-1 gap-4 sm:gap-6">
        {projects.map((project, i) => (
          ...
          >
            {/* Header */}
            <div className="flex flex-col sm:flex-row sm:items-start gap-3 sm:gap-4 p-4 sm:p-6 bg-gradient-to-r from-purple-50 to-pink-50 border-b border-gray-100">
              <div className="flex items-start gap-3 sm:gap-4 flex-1">
                ...
                <div className="flex-1 min-w-0">
                  <h3 className="text-lg sm:text-xl font-bold text-gray-900 break-words">{project.title}</h3>
                    ...
                    <div className="flex flex-wrap gap-2 mt-3">
                      {project.github && (
                        <a
                          ...
                          className="inline-flex items-center gap-1 text-xs sm:text-sm px-2 py-1 rounded-md bg-white/60 hover:bg-white border border-gray-400 hover:border-gray-700 transition-colors text-gray-700 hover:text-gray-900"
              ...
              {project.status && (
                <span
                  className={`border rounded-full px-2 sm:px-3 py-1 text-xs font-semibold self-start flex-shrink-0 ${statusStyle(
                    project.status
                  )}`}
                >
                  {project.status}
            ...
            <div className="p-4 sm:p-6">
              ...
              <div className="sm:hidden">
                ...
                {/* Content - mobile */}
                <div>
                  <p className="text-gray-600 text-sm sm:text-base mb-4 line-clamp-4">
                  ...
                  <div className="flex flex-wrap gap-1.5 sm:gap-2 mb-4">
                    {(project.tags || []).slice(0, 8).map((tag, idx) => (
                      <span
                        key={tag + idx}
                        className="px-2 sm:px-3 py-1 rounded-full text-xs bg-purple-100 text-purple-800 border border-purple-200"
                      >
                        {tag}
                      </span>
                    ))}
